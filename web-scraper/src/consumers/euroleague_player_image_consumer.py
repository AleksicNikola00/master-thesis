...
READ_QUEUE_NAME = os.getenv('LOCALSTACK_SQS_EUROLEAGUE_PLAYER_IMAGE_REQUEST')
WRITE_QUEUE_NAME = os.getenv('LOCALSTACK_SQS_EUROLEAGUE_PLAYER_IMAGE_RESPONSE')
...
class EuroleaguePlayerImageConsumer(BaseConsumer):
    def __init__(self):
        super().__init__(READ_QUEUE_URL)

    def process(self, message_content):
        url = generate_url(message_content)
